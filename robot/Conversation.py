...
from plugins.GeneFace.inference import test
...
class Conversation(object):
    ...
    def _after_play(self, msg, audios, plugin="",video_path=None):
        cached_audios = [
            f"http://{config.get('/server/host')}:{config.get('/server/port')}/audio/{os.path.basename(voice)}"
            for voice in audios
        ]
        if self.onSay:
            logger.info(f"onSay: {msg}, {cached_audios},{video_path}")
            self.onSay(msg, cached_audios, plugin=plugin,video_path=video_path)
            self.onSay = None
